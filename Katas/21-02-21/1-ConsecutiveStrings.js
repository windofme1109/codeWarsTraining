...
function longestConsec(strarr, k) {
    ...
    const consecutiveStrArr = [];

    for (let i = 0; i <= strarr.length - k; i++) {
        consecutiveStrArr.push(strarr.slice(i, i + k).join(''));
    }

    let maxLength = Math.max(...consecutiveStrArr.map(item => item.length));

    let ret = consecutiveStrArr.find(item => item.length === maxLength);
