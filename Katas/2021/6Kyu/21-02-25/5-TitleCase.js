...
function titleCase(title, minorWords) {
    const exceptionsArr = minorWords ? minorWords.split(' ').map(item => item.toLowerCase()) : [];
    const titleArr = title.split(' ');
    return titleArr.map((item, index) => {
        const lowerCaseWord = item.toLowerCase();

        if (!exceptionsArr.includes(lowerCaseWord) || index === 0) {
            return lowerCaseWord.slice(0, 1).toUpperCase() + lowerCaseWord.slice(1);
        } else if (exceptionsArr.includes(lowerCaseWord)) {
