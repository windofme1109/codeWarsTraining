...
function mean(town, strng) {
    ...
    const cityData = dataArr.find(item => item.split(':')[0] === town);

    if (cityData === undefined) {
        return -1;
    }

    const rainData = cityData.match(/\d+\.\d+/g);
    const average = rainData.reduce((acc, ele) => {
        return acc + ele * 1;
    }, 0) / 12;
...
function variance(town, strng) {
    ...
    let variance = rainData.map(data => {
        return (data * 1 - average) ** 2;
    }).reduce((acc, ele) => {
        return acc + ele;
    }, 0) / 12;
...
console.log(mean('Lima', data1));
console.log(variance('Lima', data1));
